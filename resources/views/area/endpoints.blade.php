                ...
                @foreach($module->groups as $group)
                    <div class="w-full" x-data="{open: false}">
                                ...
                                <div class="flex items-center w-full md:w-auto">
                                    ...
                                    <div class="flex space-x-2">
                                        ...
                                        <span class="text-sm px-2 flex items-center text-gray-700 bg-gray-200">
                                            {{ $group->method }}: {{ $group->route ?: $group->title }}
                                        </span>
                                    </div>
                                </div>
                        ...
                        @foreach ($group->additionalRequests as $additionalRequest)
                            <div x-cloak x-show="open" class="flex {{ $loop->first ? 'border-t' : '' }} {{ $loop->last ? '' : 'border-b' }} flex-col space-x-2 space-y-4 lg:space-y-0 lg:flex-row justify-left p-2 md:p-4 hover:bg-gray-100 border-l-8 border-gray-300">
                                <div class="flex items-center w-full md:w-auto">
                                    <div class="flex space-x-2 items-start">
